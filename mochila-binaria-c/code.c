...
int maior_peso=0,maior_beneficio=0,tam_solucao_otima=0,max=0,max2=0;
...
void bubble_sort(type_item *item, int num_items) {
    for (int i = 0; i < num_items - 1; i++) {
        for (int j = 0; j < num_items - i - 1; j++) {
            // Compara as razões benefício-peso de dois itens adjacentes
            double razao1 = item[j].beneficio / item[j].peso;
            double razao2 = item[j+1].beneficio / item[j+1].peso;
            if (razao1 < razao2) {
                // Troca os itens de posição
                type_item temp = item[j];
                item[j] = item[j+1];
                item[j+1] = temp;
            }
        }
...
void guloso(type_item *item, int num_items, int capacidad) {
    // Ordena os itens em ordem decrescente de razão benefício-peso usando o bubble sort
    bubble_sort(item, num_items);

    // Inicializa a solução
    type_item solucao[num_items];
    int armz_peso = 0;
    int soma_bene = 0;

    // Itera por todos os itens em ordem decrescente de razão benefício-peso
    for (int i = 0; i < num_items; i++) {
        // Se o item i cabe na mochila, adiciona-o à solução
        if (armz_peso + item[i].peso <= capacidad) {
            armz_peso += item[i].peso;
            soma_bene += item[i].beneficio;
            solucao[i] = item[i];
        }
    }
}
...
int main(int argc, char *argv[]) {
    ...
    printf("Digite o número do algoritmo que deseja executar:\n");
    ...
    switch (algoritmo) {
        ...
        case 2:
            ...
            printf("Tempo total guloso: %.0lf milisegundos\n", total_t_guloso);
            printf("\t\t\tPRONTO!\n\nBENEFICIO E PESO MAXIMO GULOSO: %d | %d\n", max, max2);
            break;
        case 3:
            // força bruta
            printf("\t\tExecutando o algoritmo...\n");
            ...
            printf("Tempo total forca bruta: %.0lf milisegundos\n", total_t_forca_bruta);
            printf("\t\t\tPRONTO!\n\nBENEFICIO E PESO MAXIMO FORCA BRUTA: %d | %d\n", maior_beneficio, maior_peso);
            break;
        default:
